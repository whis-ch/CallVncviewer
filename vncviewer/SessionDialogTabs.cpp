...
void SessionDialog::ReadINIProcEncoders()
{
	...
	if (config.ReadString(SWC_ENCODER, SWC_AUTODETECT, "").size() <= 0)
	{
		myINI.WriteString(SWC_ENCODER, SWC_AUTODETECT, "1");
		autoDetect = 1;
	}
	else
		autoDetect = config.ReadInt(SWC_ENCODER, SWC_AUTODETECT, 0);
	PreferredEncodings.clear();
	if (config.ReadString(SWC_ENCODER, SWC_ENCTYPE, "").size() <= 0)
	{
		myINI.WriteString(SWC_ENCODER, SWC_ENCTYPE, "0");
		PreferredEncodings.push_back(0);
	}
	else
		PreferredEncodings.push_back(config.ReadInt(SWC_ENCODER, SWC_ENCTYPE, 0) <= 0);
	...
	if (config.ReadString(SWC_ENCODER, SWC_USE8B, "").size() <= 0)
	{
		myINI.WriteString(SWC_ENCODER, SWC_USE8B, "0");
		Use8Bit = 0;
	}
	else
		Use8Bit = config.ReadInt(SWC_ENCODER, SWC_USE8B, 0);
	...
	if (config.ReadString(SWC_ENCODER, SWC_USECOMPRESSLEVEL, "").size() <= 0)
	{
		myINI.WriteString(SWC_ENCODER, SWC_USECOMPRESSLEVEL, "1");
		useCompressLevel = 1;
	}
	else
		useCompressLevel = config.ReadInt(SWC_ENCODER, SWC_USECOMPRESSLEVEL, 0);
	//compressLevel = GetDlgItemInt(hwnd, IDC_COMPRESSLEVEL, NULL, TRUE);
	if (config.ReadString(SWC_ENCODER, SWC_COMPRESSLEVEL, "").size() <= 0)
	{
		myINI.WriteString(SWC_ENCODER, SWC_COMPRESSLEVEL, "6");
		compressLevel = 6;
	}
	else
		compressLevel = config.ReadInt(SWC_ENCODER, SWC_COMPRESSLEVEL, 0);
	if (compressLevel < 0)
		compressLevel = 0;
	if (compressLevel > 9)
		compressLevel = 9;
	...
	if (config.ReadString(SWC_ENCODER, SWC_ENABLEJPEG, "").size() <= 0)
	{
		myINI.WriteString(SWC_ENCODER, SWC_ENABLEJPEG, "1");
		enableJpegCompression = 1;
	}
	else
		enableJpegCompression = config.ReadInt(SWC_ENCODER, SWC_ENABLEJPEG, 0);
	//jpegQualityLevel = GetDlgItemInt(hwnd, IDC_QUALITYLEVEL, NULL, TRUE);
	if (config.ReadString(SWC_ENCODER, SWC_JPEGLEVEL, "").size() <= 0)
	{
		myINI.WriteString(SWC_ENCODER, SWC_JPEGLEVEL, "8");
		jpegQualityLevel = 8;
	}
	else
		jpegQualityLevel = config.ReadInt(SWC_ENCODER, SWC_JPEGLEVEL, 0);
	...
	if (config.ReadString(SWC_ENCODER, SWC_PREEMPTIVEUPDATE, "").size() <= 0)
	{
		myINI.WriteString(SWC_ENCODER, SWC_PREEMPTIVEUPDATE, "0");
		preemptiveUpdates = 0;
	}
	else
		preemptiveUpdates = config.ReadInt(SWC_ENCODER, SWC_PREEMPTIVEUPDATE, 0);
	//HWND hCopyRect = GetDlgItem(hwnd, ID_SESSION_SET_CRECT);
	//UseEnc[rfbEncodingCopyRect] =
	//	(SendMessage(hCopyRect, BM_GETCHECK, 0, 0) == BST_CHECKED);
	if (config.ReadString(SWC_ENCODER, SWC_USECOPYRECTENC, "").size() <= 0)
	{
		myINI.WriteString(SWC_ENCODER, SWC_USECOPYRECTENC, "1");
		UseEnc[rfbEncodingCopyRect] = 1;
	}
	else
		UseEnc[rfbEncodingCopyRect] = config.ReadInt(SWC_ENCODER, SWC_USECOPYRECTENC, 0);
	//HWND hCache = GetDlgItem(hwnd, ID_SESSION_SET_CACHE);
	//fEnableCache =
	//	(SendMessage(hCache, BM_GETCHECK, 0, 0) == BST_CHECKED);
	if (config.ReadString(SWC_ENCODER, SWC_ENABLECACHE, "").size() <= 0)
	{
		myINI.WriteString(SWC_ENCODER, SWC_ENABLECACHE, "1");
		fEnableCache = 1;
	}
	else
		fEnableCache = config.ReadInt(SWC_ENCODER, SWC_ENABLECACHE, 0);
	//HWND hZstd = GetDlgItem(hwnd, IDC_ZSTD);
	//fEnableZstd =
	//	(SendMessage(hZstd, BM_GETCHECK, 0, 0) == BST_CHECKED);
	if (config.ReadString(SWC_ENCODER, SWC_ENABLEZTSD, "").size() <= 0)
	{
		myINI.WriteString(SWC_ENCODER, SWC_ENABLEZTSD, "1");
		fEnableZstd = 1;
	}
	else
		fEnableZstd = config.ReadInt(SWC_ENCODER, SWC_ENABLEZTSD, 0);
}
void SessionDialog::ReadINIProcKeyboardMouse()
{
	ini::iniReader config;
	bool ret = config.ReadConfig("config.ini");
	ini::IniWriter myINI(".\\config.ini");
	...
	if (config.ReadString(SWC_MOUSE_AND_KEYBOARD, SWC_VIEWONLY, "").size() <= 0)
	{
		myINI.WriteString(SWC_MOUSE_AND_KEYBOARD, SWC_VIEWONLY, "0");
		ViewOnly = 0;
	}
	else
		ViewOnly = config.ReadInt(SWC_MOUSE_AND_KEYBOARD, SWC_VIEWONLY, 0);
	//HWND hEmulate = GetDlgItem(hwnd, IDC_EMULATECHECK);
	//Emul3Buttons =
	//	(SendMessage(hEmulate, BM_GETCHECK, 0, 0) == BST_CHECKED);
	if (config.ReadString(SWC_MOUSE_AND_KEYBOARD, SWC_EMUL3BUTTON, "").size() <= 0)
	{
		myINI.WriteString(SWC_MOUSE_AND_KEYBOARD, SWC_EMUL3BUTTON, "1");
		Emul3Buttons = 1;
	}
	else
		Emul3Buttons = config.ReadInt(SWC_MOUSE_AND_KEYBOARD, SWC_EMUL3BUTTON, 0);
	//HWND hJapkeyboard = GetDlgItem(hwnd, IDC_JAPKEYBOARD);
	//JapKeyboard =
	//	(SendMessage(hJapkeyboard, BM_GETCHECK, 0, 0) == BST_CHECKED);
	if (config.ReadString(SWC_MOUSE_AND_KEYBOARD, SWC_JAPKEYBOARD, "").size() <= 0)
	{
		myINI.WriteString(SWC_MOUSE_AND_KEYBOARD, SWC_JAPKEYBOARD, "0");
		JapKeyboard = 0;
	}
	else
		JapKeyboard = config.ReadInt(SWC_MOUSE_AND_KEYBOARD, SWC_JAPKEYBOARD, 0);
	//HWND hGii = GetDlgItem(hwnd, IDC_GII);
	//giiEnable = (SendMessage(hGii, BM_GETCHECK, 0, 0) == BST_CHECKED);
	if (config.ReadString(SWC_MOUSE_AND_KEYBOARD, SWC_GIIENABLE, "").size() <= 0)
	{
		myINI.WriteString(SWC_MOUSE_AND_KEYBOARD, SWC_GIIENABLE, "0");
		giiEnable = 0;
	}
	else
		giiEnable = config.ReadInt(SWC_MOUSE_AND_KEYBOARD, SWC_GIIENABLE, 0);
	requestShapeUpdates = false;
	ignoreShapeUpdates = false;
	int RemoteCursor = 0;
	if (config.ReadString(SWC_MOUSE_AND_KEYBOARD, SWC_REMOTECURSOR, "").size() <= 0)
	{
		myINI.WriteString(SWC_MOUSE_AND_KEYBOARD, SWC_REMOTECURSOR, "1");
		RemoteCursor = 1;
	}
	else
		RemoteCursor = config.ReadInt(SWC_MOUSE_AND_KEYBOARD, SWC_REMOTECURSOR, 0);
	if (RemoteCursor == BST_CHECKED) {
		requestShapeUpdates = true;
	}
	else {
		int IgnoreRemoteCursor = 0;
		if (config.ReadString(SWC_MOUSE_AND_KEYBOARD, SWC_IGNOREREMOTE, "").size() <= 0)
		{
			myINI.WriteString(SWC_MOUSE_AND_KEYBOARD, SWC_IGNOREREMOTE, "0");
			IgnoreRemoteCursor = 0;
		}
		else
			IgnoreRemoteCursor = config.ReadInt(SWC_MOUSE_AND_KEYBOARD, SWC_IGNOREREMOTE, 0);
		if (IgnoreRemoteCursor == BST_CHECKED) {
			requestShapeUpdates = true;
	...
	if (config.ReadString(SWC_MOUSE_AND_KEYBOARD, SWC_SWAPMOUSE, "").size() <= 0)
	{
		myINI.WriteString(SWC_MOUSE_AND_KEYBOARD, SWC_SWAPMOUSE, "0");
		SwapMouse = 0;
	}
	else
		SwapMouse = config.ReadInt(SWC_MOUSE_AND_KEYBOARD, SWC_SWAPMOUSE, 0);
	if (config.ReadString(SWC_MOUSE_AND_KEYBOARD, SWC_THROTTLEMOUSE, "").size() <= 0)
	{
		myINI.WriteString(SWC_MOUSE_AND_KEYBOARD, SWC_THROTTLEMOUSE, "0");
		throttleMouse = 0;
	}
	else
		throttleMouse = config.ReadInt(SWC_MOUSE_AND_KEYBOARD, SWC_THROTTLEMOUSE, 0);
	/*BOOL bGotInt = FALSE;
	...
	if (config.ReadString(SWC_MOUSE_AND_KEYBOARD, SWC_NOHOTKEY, "").size() <= 0)
	{
		myINI.WriteString(SWC_MOUSE_AND_KEYBOARD, SWC_NOHOTKEY, "0");
		NoHotKeys = 0;
	}
	else
		NoHotKeys = config.ReadInt(SWC_MOUSE_AND_KEYBOARD, SWC_NOHOTKEY, 0);
}
void SessionDialog::ReadINIProcDisplay()
{
	ini::iniReader config;
	bool ret = config.ReadConfig("config.ini");
	ini::IniWriter myINI(".\\config.ini");
	...
	//HWND hwnd = DisplayHwnd;
	//HWND hShowToolbar = GetDlgItem(hwnd, IDC_SHOWTOOLBAR);
	//ShowToolbar = (SendMessage(hShowToolbar, BM_GETCHECK, 0, 0) == BST_CHECKED);
	if (config.ReadString(SWC_DISPLAY, SWC_SHOWTOOLBAR, "").size() <= 0)
	{
		myINI.WriteString(SWC_DISPLAY, SWC_SHOWTOOLBAR, "0");
		ShowToolbar = 0;
	}
	else
		ShowToolbar = config.ReadInt(SWC_DISPLAY, SWC_SHOWTOOLBAR, 0);
	//HWND hAutoScaling = GetDlgItem(hwnd, IDC_SCALING);
	//fAutoScaling = (SendMessage(hAutoScaling, BM_GETCHECK, 0, 0) == BST_CHECKED);
	if (config.ReadString(SWC_DISPLAY, SWC_AUTOSCALING, "").size() <= 0)
	{
		myINI.WriteString(SWC_DISPLAY, SWC_AUTOSCALING, "0");
		fAutoScaling = 0;
	}
	else
		fAutoScaling = config.ReadInt(SWC_DISPLAY, SWC_AUTOSCALING, 0);
	/*HWND hAutoScalingEven = GetDlgItem(hwnd, IDC_SCALINGEVEN);
	fAutoScalingEven = (SendMessage(hAutoScalingEven, BM_GETCHECK, 0, 0) == BST_CHECKED);*/
	if (config.ReadString(SWC_DISPLAY, SWC_AUTOSCALINGEVEN, "").size() <= 0)
	{
		myINI.WriteString(SWC_DISPLAY, SWC_AUTOSCALINGEVEN, "0");
		fAutoScalingEven = 0;
	}
	else
		fAutoScalingEven = config.ReadInt(SWC_DISPLAY, SWC_AUTOSCALINGEVEN, 0);
	//HWND hViewerScaling = GetDlgItem(hwnd, IDC_SCALE_CB);
	/*int nErr;
	int nPer = GetDlgItemInt(hwnd, IDC_SCALE_CB, &nErr, FALSE);*/
	int nPer = 0;
	if (config.ReadString(SWC_DISPLAY, SWC_PER, "").size() <= 0)
	{
		myINI.WriteString(SWC_DISPLAY, SWC_PER, "100");
		nPer = 100;
	}
	else
		nPer = config.ReadInt(SWC_DISPLAY, SWC_PER, 0);
	if (nPer > 0) {
		scale_num = nPer;
		scale_den = 100;
	}
	...
	if (scaling || fAutoScaling) {
		...
		if (config.ReadString(SWC_DISPLAY, SWC_SERVERSCALE, "").size() <= 0)
		{
			myINI.WriteString(SWC_DISPLAY, SWC_SERVERSCALE, "1");
			nServerScale = 1;
		}
		else
			nServerScale = config.ReadInt(SWC_DISPLAY, SWC_SERVERSCALE, 0);
		FixScaling();
	}
	else {
		scale_num = 1;
	...
	//HWND hFullScreen = GetDlgItem(hwnd, IDC_FULLSCREEN);
	//FullScreen =(SendMessage(hFullScreen, BM_GETCHECK, 0, 0) == BST_CHECKED);
	if (config.ReadString(SWC_DISPLAY, SWC_FULLSCREEN, "").size() <= 0)
	{
		myINI.WriteString(SWC_DISPLAY, SWC_FULLSCREEN, "0");
		FullScreen = 0;
	}
	else
		FullScreen = config.ReadInt(SWC_DISPLAY, SWC_FULLSCREEN, 0);
	//HWND hSavePos = GetDlgItem(hwnd, IDC_SAVEPOS);
	//SavePos =(SendMessage(hSavePos, BM_GETCHECK, 0, 0) == BST_CHECKED);
	if (config.ReadString(SWC_DISPLAY, SWC_SAVEPOS, "").size() <= 0)
	{
		myINI.WriteString(SWC_DISPLAY, SWC_SAVEPOS, "0");
		SavePos = 0;
	}
	else
		SavePos = config.ReadInt(SWC_DISPLAY, SWC_SAVEPOS, 0);
	//HWND hSaveSize = GetDlgItem(hwnd, IDC_SAVESIZE);
	//SaveSize =(SendMessage(hSaveSize, BM_GETCHECK, 0, 0) == BST_CHECKED);
	if (config.ReadString(SWC_DISPLAY, SWC_SAVESIZE, "").size() <= 0)
	{
		myINI.WriteString(SWC_DISPLAY, SWC_SAVESIZE, "0");
		SaveSize = 0;
	}
	else
		SaveSize = config.ReadInt(SWC_DISPLAY, SWC_SAVESIZE, 0);
	/*HWND hDirectx = GetDlgItem(hwnd, IDC_DIRECTX);
	Directx =(SendMessage(hDirectx, BM_GETCHECK, 0, 0) == BST_CHECKED);*/
	if (config.ReadString(SWC_DISPLAY, SWC_DIRECTX, "").size() <= 0)
	{
		myINI.WriteString(SWC_DISPLAY, SWC_DIRECTX, "1");
		Directx = 1;
	}
	else
		Directx = config.ReadInt(SWC_DISPLAY, SWC_DIRECTX, 0);
	//HWND hAllowMonitorSpanning = GetDlgItem(hwnd, IDC_ALLOWSPAN);
	//allowMonitorSpanning =(SendMessage(hAllowMonitorSpanning, BM_GETCHECK, 0, 0) == BST_CHECKED);
	if (config.ReadString(SWC_DISPLAY, SWC_ALLOWMONITOR, "").size() <= 0)
	{
		myINI.WriteString(SWC_DISPLAY, SWC_ALLOWMONITOR, "0");
		allowMonitorSpanning = 0;
	}
	else
		allowMonitorSpanning = config.ReadInt(SWC_DISPLAY, SWC_ALLOWMONITOR, 0);
	/*HWND hChangeServerRes = GetDlgItem(hwnd, IDC_CHANGESERVER);
	changeServerRes =(SendMessage(hChangeServerRes, BM_GETCHECK, 0, 0) == BST_CHECKED);*/
	if (config.ReadString(SWC_DISPLAY, SWC_CHANGESERVERRES, "").size() <= 0)
	{
		myINI.WriteString(SWC_DISPLAY, SWC_CHANGESERVERRES, "0");
		changeServerRes = 0;
	}
	else
		changeServerRes = config.ReadInt(SWC_DISPLAY, SWC_CHANGESERVERRES, 0);
	//HWND hExtendDisplay = GetDlgItem(hwnd, IDC_RADIO_EXTEND);
	//extendDisplay =(SendMessage(hExtendDisplay, BM_GETCHECK, 0, 0) == BST_CHECKED);
	if (config.ReadString(SWC_DISPLAY, SWC_EXTENDDISPLAY, "").size() <= 0)
	{
		myINI.WriteString(SWC_DISPLAY, SWC_EXTENDDISPLAY, "0");
		extendDisplay = 0;
	}
	else
		extendDisplay = config.ReadInt(SWC_DISPLAY, SWC_EXTENDDISPLAY, 0);
	//HWND hShowExtend = GetDlgItem(hwnd, IDC_SHOW_EXTEND);
	//showExtend =(SendMessage(hShowExtend, BM_GETCHECK, 0, 0) == BST_CHECKED);
	if (config.ReadString(SWC_DISPLAY, SWC_SHOWEXTEND, "").size() <= 0)
	{
		myINI.WriteString(SWC_DISPLAY, SWC_SHOWEXTEND, "0");
		showExtend = 0;
	}
	else
		showExtend = config.ReadInt(SWC_DISPLAY, SWC_SHOWEXTEND, 0);
	//HWND hUseVirt = GetDlgItem(hwnd, IDC_RADIO_ONLY_VIRTUAL);
	//use_virt =(SendMessage(hUseVirt, BM_GETCHECK, 0, 0) == BST_CHECKED);
	if (config.ReadString(SWC_DISPLAY, SWC_USERVIRT, "").size() <= 0)
	{
		myINI.WriteString(SWC_DISPLAY, SWC_USERVIRT, "0");
		use_virt = 0;
	}
	else
		use_virt = config.ReadInt(SWC_DISPLAY, SWC_USERVIRT, 0);
	//HWND hAllMons = GetDlgItem(hwnd, IDC_ALLMONS);
	//useAllMonitors =(SendMessage(hAllMons, BM_GETCHECK, 0, 0) == BST_CHECKED);
	if (config.ReadString(SWC_DISPLAY, SWC_USEALLMONITOR, "").size() <= 0)
	{
		myINI.WriteString(SWC_DISPLAY, SWC_USEALLMONITOR, "0");
		ShowToolbar = 0;
	}
	else
		useAllMonitors = config.ReadInt(SWC_DISPLAY, SWC_USEALLMONITOR, 0);
}
void SessionDialog::ReadINIProcMisc()
{
	ini::IniWriter myINI(".\\config.ini");
	ini::iniReader config;
	bool ret = config.ReadConfig("config.ini");
	...
	HWND hwnd = MiscHwnd;
	//strcpy_s(folder, 256, config.ReadString(SWC_MISC, "folder", "").c_str());
	if (config.ReadString(SWC_MISC, SWC_PREFIX, "").size() <= 0)
	{
		myINI.WriteString(SWC_MISC, SWC_PREFIX, "vnc_");
		strcpy_s(prefix, 56, "vnc_");
	}
	else 
		strcpy_s(prefix, 56, config.ReadString(SWC_MISC, SWC_PREFIX, "").c_str());
	UINT res = GetDlgItemText(hwnd, IDC_FOLDER, folder, 256);
	//res = GetDlgItemText(hwnd, IDC_PREFIX, prefix, 56);
	if (Shared)
	{
		if (config.ReadString(SWC_MISC, SWC_RECONNECTCOUNTER, "").size() <= 0)
		{
			myINI.WriteString(SWC_MISC, SWC_RECONNECTCOUNTER, "3");
			ShowToolbar = 3;
		}
		else
			reconnectcounter = config.ReadInt(SWC_MISC, SWC_RECONNECTCOUNTER, 0);
	}
	...
	if (Shared)
	{
		if (config.ReadString(SWC_MISC, SWC_AUTORECONNECT, "").size() <= 0)
		{
			myINI.WriteString(SWC_MISC, SWC_AUTORECONNECT, "3");
			autoReconnect = 3;
		}
		else
			autoReconnect = config.ReadInt(SWC_MISC, SWC_AUTORECONNECT, 0);
	}
	else
		autoReconnect = 0;
	/*if (Shared)
	...
	if (config.ReadString(SWC_MISC, SWC_FTTIMEOUT, "").size() <= 0)
	{
		myINI.WriteString(SWC_MISC, SWC_FTTIMEOUT, "30");
		FTTimeout = 30;
	}
	else
		FTTimeout = config.ReadInt(SWC_MISC, SWC_FTTIMEOUT, 0);
	//HWND hsponsor = GetDlgItem(hwnd, IDC_CHECK1);
	//if (SendMessage(hsponsor, BM_GETCHECK, 0, 0) == BST_CHECKED)
	//	g_disable_sponsor = true;
	//else
	//	g_disable_sponsor = false;
	if (config.ReadString(SWC_MISC, SWC_DISABLESPONSOR, "").size() <= 0)
	{
		myINI.WriteString(SWC_MISC, SWC_DISABLESPONSOR, "0");
		g_disable_sponsor = 0;
	}
	else
		g_disable_sponsor = config.ReadInt(SWC_MISC, SWC_DISABLESPONSOR, 0);
	/*HWND hNoStatus = GetDlgItem(hwnd, IDC_HIDESTATUS);
	NoStatus = (SendMessage(hNoStatus, BM_GETCHECK, 0, 0) == BST_CHECKED);*/
	if (config.ReadString(SWC_MISC, SWC_NOSTATUS, "").size() <= 0)
	{
		myINI.WriteString(SWC_MISC, SWC_NOSTATUS, "0");
		NoStatus = 0;
	}
	else
		NoStatus = config.ReadInt(SWC_MISC, SWC_NOSTATUS, 0);
	//GetDlgItemText(hwnd, IDC_IMAGEFORMAT, imageFormat, 56);
	if (config.ReadString(SWC_MISC, SWC_IMAGEFORMAT, "").size() <= 0)
	{
		myINI.WriteString(SWC_MISC, SWC_IMAGEFORMAT, ".jpeg");
		strcpy_s(imageFormat, 56, ".jpeg");
	}
	else
	strcpy_s(imageFormat, 56, config.ReadString(SWC_MISC, SWC_IMAGEFORMAT, "").c_str());
}
void SessionDialog::ReadINIProcSecurity()
{
	ini::iniReader config;
	ini::IniWriter myINI(".\\config.ini");
	bool ret = config.ReadConfig("config.ini");
	...
	if (config.ReadString(SWC_SECURITY, SWC_SHAERD, "").size() <= 0)
	{
		myINI.WriteString(SWC_SECURITY, SWC_SHAERD, "1");
		Shared = 1;
	}
	else
		Shared = config.ReadInt(SWC_SECURITY, SWC_SHAERD, 0);
	//DisableClipboard = (SendMessage(GetDlgItem(hwnd, IDC_DISABLECLIPBOARD), BM_GETCHECK, 0, 0) == BST_CHECKED);
	if (config.ReadString(SWC_SECURITY, SWC_DISABLECLIPBOARD, "").size() <= 0)
	{
		myINI.WriteString(SWC_SECURITY, SWC_DISABLECLIPBOARD, "0");
		DisableClipboard = 0;
	}
	else
		DisableClipboard = config.ReadInt(SWC_SECURITY, SWC_DISABLECLIPBOARD, 0);
	...
	if (config.ReadString(SWC_SECURITY, SWC_USEENCRYPTION, "").size() <= 0)
	{
		myINI.WriteString(SWC_SECURITY, SWC_USEENCRYPTION, "0");
		useEncryption = 0;
	}
	else
		useEncryption = config.ReadInt(SWC_SECURITY, SWC_USEENCRYPTION, 0);

	if (useEncryption) {
		//TCHAR szPlugin[MAX_PATH];
		//GetDlgItemText(hwnd, IDC_PLUGINS_COMBO, szPlugin, MAX_PATH);
		fUseDSMPlugin = true;
		if (config.ReadString(SWC_SECURITY, SWC_DSMPLUGINFILENAME, "").size() <= 0)
		{
			myINI.WriteString(SWC_SECURITY, SWC_DSMPLUGINFILENAME, "No Plugin detected...");
			strcpy_s(imageFormat, 56, "No Plugin detected...");
		}
		else
			strcpy_s(szDSMPluginFilename, config.ReadString(SWC_SECURITY, SWC_DSMPLUGINFILENAME, "").c_str());
	}
	else
		fUseDSMPlugin = false;
	//fRequireEncryption = (SendMessage(GetDlgItem(hwnd, IDC_ONLYENCRYPTED), BM_GETCHECK, 0, 0) == BST_CHECKED);
	if (config.ReadString(SWC_SECURITY, SWC_REQUIREENCRYPTION, "").size() <= 0)
	{
		myINI.WriteString(SWC_SECURITY, SWC_REQUIREENCRYPTION, "0");
		fRequireEncryption = 0;
	}
	else
		fRequireEncryption = config.ReadInt(SWC_SECURITY, SWC_REQUIREENCRYPTION, 0);
	//AllowUntrustedServers = !(SendMessage(GetDlgItem(hwnd, IDC_ONLYPASSWORD), BM_GETCHECK, 0, 0) == BST_CHECKED);
	if (config.ReadString(SWC_SECURITY, SWC_ALLOWUNTRUSTEDSERVER, "").size() <= 0)
	{
		myINI.WriteString(SWC_SECURITY, SWC_ALLOWUNTRUSTEDSERVER, "1");
		AllowUntrustedServers =1;
	}
	else
		AllowUntrustedServers = !(config.ReadInt(SWC_SECURITY, SWC_ALLOWUNTRUSTEDSERVER, 0) == 1);
	//fAutoAcceptIncoming = (SendMessage(GetDlgItem(hwnd, IDC_AUTOACCEPT), BM_GETCHECK, 0, 0) == BST_CHECKED);
	if (config.ReadString(SWC_SECURITY, SWC_AUTOACCEPTINCOMING, "").size() <= 0)
	{
		myINI.WriteString(SWC_SECURITY, SWC_AUTOACCEPTINCOMING, "0");
		fAutoAcceptIncoming = 0;
	}
	else
		fAutoAcceptIncoming = config.ReadInt(SWC_SECURITY, SWC_AUTOACCEPTINCOMING, 0);
	//fAutoAcceptNoDSM = (SendMessage(GetDlgItem(hwnd, IDC_AUTOACCEPTNOWARN), BM_GETCHECK, 0, 0) == BST_CHECKED);
	if (config.ReadString(SWC_SECURITY, SWC_AUTOACCEPTNODSM, "").size() <= 0)
	{
		myINI.WriteString(SWC_SECURITY, SWC_AUTOACCEPTNODSM, "0");
		fAutoAcceptNoDSM = 0;
	}
	else
		fAutoAcceptNoDSM = config.ReadInt(SWC_SECURITY, SWC_AUTOACCEPTNODSM, 0);
	//restricted = (SendMessage(GetDlgItem(hwnd, IDC_HIDEMENU), BM_GETCHECK, 0, 0) == BST_CHECKED);
	if (config.ReadString(SWC_SECURITY, SWC_RESTRICT, "").size() <= 0)
	{
		myINI.WriteString(SWC_SECURITY, SWC_RESTRICT, "0");
		restricted = 0;
	}
	else
		restricted = config.ReadInt(SWC_SECURITY, SWC_RESTRICT, 0);
	//GetDlgItemText(hwnd, IDC_EDITCUSTOMMESSAGE, InfoMsg, 255);
	if (config.ReadString(SWC_SECURITY, SWC_INFOMSG, "").size() <= 0)
	{
		myINI.WriteString(SWC_SECURITY, SWC_INFOMSG, "");
		strcpy_s(InfoMsg, 255, "");
	}
	else
		strcpy_s(InfoMsg, 255, config.ReadString(SWC_SECURITY, SWC_INFOMSG, "").c_str());
}
void SessionDialog::ReadINIProc()
{
	ini::IniWriter myINI(".\\config.ini");
	ini::iniReader config;
	bool ret = config.ReadConfig("config.ini");
	...
	if (config.ReadString(SWC_GLOBAL, SWC_DIALOG, "").size() <= 0)
	{
		myINI.WriteString(SWC_GLOBAL, SWC_DIALOG, "192.168.1.247");
		strcpy_s(hostname, 256, "192.168.1.247");
	}
	else
		strcpy_s(hostname, 256, config.ReadString(SWC_GLOBAL, SWC_DIALOG, "").c_str());
	...
	if (config.ReadString(SWC_GLOBAL, SWC_HOST, "").size() <= 0)
	{
		myINI.WriteString(SWC_GLOBAL, SWC_HOST, "");
		strcpy_s(hostname, 256, "");
	}
	else
		strcpy_s(hostname, 256, config.ReadString(SWC_GLOBAL, SWC_HOST, "").c_str());
	//adzm 2010-02-15
	...
	if (config.ReadString(SWC_GLOBAL, SWC_USEPROXY, "").size() <= 0)
	{
		myINI.WriteString(SWC_GLOBAL, SWC_USEPROXY, "0");
		m_fUseProxy =0;
	}
	else
		m_fUseProxy = config.ReadInt(SWC_GLOBAL, SWC_USEPROXY, 0);
}
void SessionDialog::ReadINIProcListen()
{
	ini::IniWriter myINI(".\\config.ini");
	ini::iniReader config;
	bool ret = config.ReadConfig("config.ini");
	...
	if (config.ReadString(SWC_LISTEN, SWC_LISTENPORT, "").size() <= 0)
	{
		myINI.WriteString(SWC_LISTEN, SWC_LISTENPORT, "5900");
		listenport = 5900;
	}
	else
		listenport = config.ReadInt(SWC_LISTEN, SWC_LISTENPORT, 0);
}
////////////////////////////////////////////////////////////////////////////////
