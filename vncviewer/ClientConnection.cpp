...
void ClientConnection::AuthenticateServer(CARD32 authScheme, std::vector<CARD32>&current_auth)
{
	...
	case rfbNoAuth:
		...
		/*	if (!m_Is_Listening && !m_pApp->m_options.m_AllowUntrustedServers  && MessageBox(m_hwndMain, "The Server has been setup without authentication, do you trust this server?", "Accept server without authentification", MB_YESNO | MB_ICONEXCLAMATION | MB_TOPMOST) == IDNO)
			...
			//隐藏界面
		if (!m_Is_Listening && !m_pApp->m_options.m_AllowUntrustedServers && 0)
		{
			throw WarningException("You refused a untrusted server.");
		}
...
{
	//截取键值
	if (virtKey == 101)//输入numpad5修改为clear即(select)按键
	{
		virtKey = 12;
		keyData = 4980737;
	}
	bool leftShift = (GetKeyState(VK_LSHIFT) & 0x8000) != 0;
	bool rightShift = (GetKeyState(VK_RSHIFT) & 0x8000) != 0;
	bool shift = (GetKeyState(VK_SHIFT) & 0x8000) != 0;
	bool sub = ((GetKeyState(VK_SUBTRACT) & 0x8000) != 0);
	bool dev = ((GetKeyState(VK_DIVIDE) & 0x8000) != 0);
	bool add = ((GetKeyState(VK_ADD) & 0x8000) != 0);
	bool mul = ((GetKeyState(VK_MULTIPLY) & 0x8000) != 0);
	if ((leftShift && sub) || (rightShift && sub) || (leftShift && sub))//'/'改为'?'
	{
		virtKey = 189;
		keyData = 786433;
	}
	else if ((leftShift && dev) || (rightShift && dev) || (leftShift && dev))//'+'改为'~'
	{
		virtKey = 191;
		keyData = 3473409;
	}
	else if ((leftShift && add) || (rightShift && add) || (leftShift && add))//'*'改为'`'
	{
		virtKey = 192;
		keyData = 2686977;
	}
	else if ((leftShift && mul) || (rightShift && mul) || (leftShift && mul))//'-'改为'_'
	{
		/*virtKey = 192;
		keyData = 3604481;*/
...
void ClientConnection::GTGBS_CreateDisplay()
{
	...
	DWORD winstyle;
	if (config.ReadString(SWC_GLOBAL, SWC_USETITALBAR, "").size() <= 0)
	{
		myINI.WriteString(SWC_GLOBAL, SWC_USETITALBAR, "0");
		winstyle = WS_POPUP;
	}
	else
	{
		int useTitalBar = config.ReadInt(SWC_GLOBAL, SWC_USETITALBAR, 0);
		if (useTitalBar)
		{
...
LRESULT CALLBACK ClientConnection::WndProc(HWND hwnd, UINT iMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_SYSCHAR:
		return true;
	case WM_COPYDATA://进程间消息通讯
	{
		COPYDATASTRUCT* copy_data_structure = { 0 };
		copy_data_structure = (COPYDATASTRUCT*)lParam;
		if (copy_data_structure->dwData == communication::CUSTOM_TYPE)
		{
			std::string str = (char*)copy_data_structure->lpData;//接受的消息
			communication::communicationToOthers comnumicate;
			comnumicate.MessageReceive(str);
		}
	}
	break;
	case WM_SYSCOMMAND:
